...
`fred.workspaces` runs on stock Omarchy. Two behaviours it depends on, though,
...
[omarchy-fred-ecosystem](https://github.com/greenermoose/omarchy-fred-ecosystem)
(`ECOSYSTEM.md` there is the full matrix).

| Package | Stock behaviour | With the patch | Where |
|---|---|---|---|
| **hyprland** ≥ 0.56.2 — [`patch/idle-notify-inhibit-unchanged-noop`](https://github.com/greenermoose/Hyprland/tree/patch/idle-notify-inhibit-unchanged-noop) | This plugin's hotplug `reconcile` (and anything else that focuses a window) re-evaluates Hyprland's idle-inhibit state; unchanged or not, Hyprland reset every `ext-idle-notify` timer, so a monitor's routine HPD drop ~7 s after DPMS-off counted as user activity and hypridle never reached its suspend rule (85 blank/resume cycles in one night). | An unchanged inhibit state is a no-op; only input and real inhibitor transitions touch the timers. | [registry entry](https://github.com/greenermoose/omarchy-fred-ecosystem/blob/main/packages/hyprland/README.md) · [compare](https://github.com/hyprwm/Hyprland/compare/v0.56.2...greenermoose:Hyprland:v0.56.2-fred.2) |
...
every hotplug, this is why; the fix is 7 lines and lives on the fork branch
above, built with the recipe in the registry.
