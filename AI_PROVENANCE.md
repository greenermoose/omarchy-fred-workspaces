...
| **Per-Monitor Idle Blanking of Unused Monitors** | `v1.5.0` | Antigravity CLI `1.2.3` (Gemini 3.8 Flash (High)) | Strictly event-driven per-monitor idle DPMS blanking embedded directly in `fred.workspaces` BarWidget; zero background polling preserving CPU deep C-states; immediate wake on cursor entry / window / workspace change; Fault C protection on DP-2 with background retrain; remote `resetIdle` IPC method. |
