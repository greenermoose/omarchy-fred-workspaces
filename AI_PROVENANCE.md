...
| **Split Monitor Sets: Partial State & Follow Focus** | `v1.4.2` *(Unreleased)* | Claude Code `2.1.273` (Claude Opus 5) | `splitSet` setting: `partial` (default) marks only the monitor that followed a focus with a hollow marker and an **F** mode letter, and click returns that monitor to the set's desktop; `follow` realigns the set automatically on focus changes. |
