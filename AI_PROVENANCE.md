...
| **Split Monitor Sets: Partial State & Follow Focus** | `v1.4.2` *(Unreleased)* | Claude Code `2.1.273` (Claude Opus 5) | `splitSet` setting: `partial` (default) marks the focused monitor's desktop, turns the mode letter to **P**, and click-realigns to the pre-split desktop; `follow` realigns the set automatically on focus changes. |
